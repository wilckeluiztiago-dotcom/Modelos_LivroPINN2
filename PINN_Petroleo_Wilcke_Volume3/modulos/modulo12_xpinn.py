...
# Capítulo 12
# =============================================================================
"""Capítulo 12 - Decomposição de domínio, condições de interface"""
...
class XPINN:
    ...
    def __init__(self):
        ...
        self.cfg = FISICA
        self.geo = GEOMETRIA
        LOGGER.info("XPINN inicializado por Luiz Tiago Wilcke")
    ...
    def resumo(self) -> Dict:
        return {
            ...
            "descricao": "Capítulo 12 - Decomposição de domínio, condições de interface"
        }

class PINNXPINN(RedeBasePINN):
...
# ---------------------------------------------------------------------------
# Funções auxiliares específicas do módulo XPINN
# ---------------------------------------------------------------------------

def gerar_pontos_colocacao_xpinn(n_pontos: int = 5000, dominio: Tuple = (0, 1)) -> torch.Tensor:
    """Gera pontos de colocação no domínio físico."""
    return torch.rand(n_pontos, 2) * (dominio[1] - dominio[0]) + dominio[0]

def validar_solucao_xpinn(pred: np.ndarray, tolerancia: float = 1e-3) -> bool:
...
def exportar_resultados_xpinn(dados: Dict, caminho: str) -> None:
